...
from ConectBD.ConexaoBancoDeDados import Base, Pessoa
...
class PostoConvenido(Base):
    ...
    def Cadastrar():
        nome_posto = InputiNomeP.get()
        cnpj_posto = inputCNPJ.get()
        codigo_posto = icd_posto.get()
        data_convenio = inputData.get()
        status_posto = inputStatus.get()


        Session.execute(Text("INSERT INTO ABASTECIMENTO (cd_posto, nm_posto, cnpj, dt_convenio, in_ativo) VALUES (?, ?, ?, ?, ?)",
                    (codigo_posto, nome_posto, cnpj_posto, data_convenio, status_posto)))
    ...
    #    Label(janela, image=img).pack()
        ...
        botao_cadastrar = Button(janela, text="Salvar", padx=50, pady=10, bd=5, font=("Arial", 12), bg="#1B492B", command=Cadastrar)
